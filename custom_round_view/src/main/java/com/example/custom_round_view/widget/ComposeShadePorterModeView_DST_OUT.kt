 ...
 */
class ComposeShadePorterModeView_DST_OUT(
    context: Context?,
    ...
    private fun drawText(canvas: Canvas?) {
        canvas?.apply {
            ...
            var title = "相交处根据源图像alpha进行过滤，完全不透明处则完全过滤，完全透明则不过滤。"

            val fontMetrics: Paint.FontMetrics = paint.fontMetrics
            val distance = (fontMetrics.bottom - fontMetrics.top) / 2 - fontMetrics.bottom
            val baseline: Float = height.toFloat() / 2 + distance
            drawText(title, width / 2f, baseline + height / 4 + 50f, paint)
            drawText("DST_OUT：对比SRC_OUT , 在不相交的地方绘制目标图像，", width / 2f, baseline + height / 4, paint)
        }
    }
